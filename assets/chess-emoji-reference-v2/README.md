...
Unlike the first palette, every file is a complete board cell: the tile
background, optional selection/legal/capture marker, and optional chess piece
are composited into the same sprite. That keeps every board position on one
fixed visual grid in Telegram.
